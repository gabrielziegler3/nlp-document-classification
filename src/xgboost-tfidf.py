...
tfidf = pd.read_csv("../data/TF-IDF-24-08-2018.csv", index_col=0)
tfidf_words = tfidf.iloc[:, :27]
print(len(tfidf_words.columns), tfidf_words.columns)
tfidf_scores = tfidf.iloc[:, 27:]
print(len(tfidf_scores.columns), tfidf_scores.columns)

n_words = 2000
vocab = []
for i in tfidf_words.columns:
    vocab.extend(tfidf[i].iloc[:n_words].astype(str).tolist())
